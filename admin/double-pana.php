...
if(isset($_POST['submit'])){
  ...
  $double_pana = $_POST['double_pana'];
  $createDate = date("Y-m-d H:i:s");
  
  $select = mysqli_query($con, "SELECT *FROM `double_pana` WHERE `single_ank`='$single_ank' AND `double_pana`='$double_pana' ");
  $count = mysqli_num_rows($select);
  
  if($count > 0){
      echo "<script>alert('Already exits. Please try again...!');</script>";
  }else{
      $insert = mysqli_query($con, "INSERT INTO `double_pana`(`single_ank`, `double_pana`, `created_at`) VALUES ('$single_ank','$double_pana','$createDate')");
  
      if($insert){
          echo "<script>window.location.href='double-pana.php';</script>";
      }
  }
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Double Pana</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                ?>
                  ...
                  <div class="row">
                    <?php
                      $singlePana = mysqli_query($con, "SELECT * FROM `double_pana` WHERE `single_ank`='$single' ORDER BY double_pana ASC ");
                      while($fetch = mysqli_fetch_array($singlePana)){
                    ?>
                    <div class="col-md-1">
                      <span class="info-box-icon badge badge-primary"><?php echo $fetch['double_pana']; ?></span>
                    </div>
                    <?php
                      }
                    ?>
