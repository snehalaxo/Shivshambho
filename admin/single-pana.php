...
if(isset($_POST['submit'])){
  ...
  $single_pana = $_POST['single_pana'];
  $createDate = date("Y-m-d H:i:s");
  
  $select = mysqli_query($con, "SELECT *FROM `single_pana` WHERE `single_ank`='$single_ank' AND `single_pana`='$single_pana' ");
  $count = mysqli_num_rows($select);
  
  if($count > 0){
      echo "<script>alert('Already exits. Please try again...!');</script>";
  }else{
      $insert = mysqli_query($con, "INSERT INTO `single_pana`(`single_ank`, `single_pana`, `created_at`) VALUES ('$single_ank','$single_pana','$createDate')");
  
      if($insert){
          echo "<script>window.location.href='single-pana.php';</script>";
      }
  }
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Single Pana</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                ?>
                  ...
                  <div class="row">
                    <?php
                      $singlePana = mysqli_query($con, "SELECT * FROM `single_pana` WHERE `single_ank`='$single' ORDER BY single_pana ASC ");
                      while($fetch = mysqli_fetch_array($singlePana)){
                    ?>
                    <div class="col-md-1">
                      <span class="info-box-icon badge badge-primary"><?php echo $fetch['single_pana']; ?></span>
                    </div>
                    <?php
                      }
                    ?>
